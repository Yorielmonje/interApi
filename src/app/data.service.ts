...
export class DataService {
  ...
  private goals = new BehaviorSubject<any>(['jugos']);
  ...
  apiURL = 'http://34.125.7.41:8120/yorielme-api';
  ...
  getGoals(): Observable<GoalsApi> {
    return this.http.get<GoalsApi>(this.apiURL + '/jugos', this.httpOptions)
    .pipe(
      retry(1),
  ...
  newGoal(payload): Observable<GoalsApi> {
    return this.http.post<GoalsApi>(this.apiURL + '/jugos', JSON.stringify(payload), this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
